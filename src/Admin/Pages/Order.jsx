                                ...
                                <tr key={order.id}>
                                    ...
                                    <td>
                                        {order.items && order.items.length > 0 ? (
                                            order.items.map((item) => (
                                                <div key={item.id} style={{  marginBottom: '10px',   borderBottom: '1px solid white' }}>
                                                    {item.name} <span style={{ color: 'red', margin: '0 5px' }}>x</span> {item.quantity} <span style={{ color: 'red', margin: '0 5px' }}>x</span> {item.total.toLocaleString("uk-UA")} грн
                                                </div>
                                            ))
                                        ) : (
                                    ...
                                    <td>
                                        <Link to={`/admin-panel/order/${order.id}`}>
                                            <button className="details-button">Деталі</button>
                                        </Link>
                                    </td>
                                </tr>
                            ))}
